...
const tamanhosPorUnidade = {
  ...
  'Comando Regional': { area: 300, categoria: 'pequeno' }
};
...
const RiskMatrixApp = () => {
  ...
  // Função para obter informações da unidade atual
  ...
  const getAdvancedRiskCriteria = (projData) => {
    ...
    switch (tamanhoInfo.categoria) {
      ...
      case 'grande':
        ...
        break;
      case 'muito_grande':
        criteria.additional_risks.push(...[1, 4, 9, 23, 37, 46]); // Todos os riscos
    ...
    projData.caracteristicas.forEach(caracteristica => {
      switch (caracteristica) {
        ...
        case 'Integração com sistemas existentes':
          criteria.additional_risks.push(...[14, 29, 34, 44, 65]);
          criteria.debug_logic.push('+ Riscos de integração de sistemas');
          break;
      }
    });
